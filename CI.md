...
| [`.github/workflows/docs-pages.yml`](.github/workflows/docs-pages.yml) | Push to `main`, `workflow_dispatch` | Build customer MkDocs handbook ([`doc/customer/mkdocs.yml`](doc/customer/mkdocs.yml)) with **`mkdocs build --strict`**, deploy static site to **GitHub Pages** |
...
### Workflow: `docs-pages.yml` (customer handbook → GitHub Pages)

- **Trigger**: `push` to **`main`** (full branch — republishes on every merge) and manual **`workflow_dispatch`**.
- **Permissions**: `contents: read`, **`pages: write`**, **`id-token: write`** (required by Pages deployment).
- **Jobs**: **`build-docs`** installs MkDocs + Material + pymdown via **`pip`**, runs **`mkdocs build --strict --config-file doc/customer/mkdocs.yml`**, uploads **`doc/customer/site/`** with **`actions/upload-pages-artifact@v3`**; **`deploy-docs`** runs **`actions/deploy-pages@v4`** against the **`github-pages`** environment.
- **Repository setup**: under **Settings → Pages**, set **Source** to **GitHub Actions** (not `gh-pages` branch). Published URL follows **`site_url`** in [`doc/customer/mkdocs.yml`](doc/customer/mkdocs.yml) (currently **`https://bastiion.github.io/tutor-certification/`**).

**Local equivalent** (strict build + image ref check, matches toolchain intent):
...
flowchart TD
  ...
  subgraph docsPages [docs-pages.yml]
    BD[build-docs]
    DD[deploy-docs]
    BD --> DD
  end
...
| [`.github/workflows/docs-pages.yml`](.github/workflows/docs-pages.yml) | MkDocs → GitHub Pages |
